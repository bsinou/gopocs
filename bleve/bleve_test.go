...
func TestFillBleveDB(t *testing.T) {
	...
	Convey(fmt.Sprintf("Given a test corpus of %dk log messages", loopNb), t, func() {
		...
		Convey("Index in the default store", func() {
			var err error
			sname := "defaultStore"
			path := fmt.Sprintf("%s/test-%dk-%s.bleve", workingDir, loopNb, sname)
			server, err := NewDefaultServer(path)
			if err != nil {
		...
		Convey("Index in a default store with mapping", func() {
			var err error
		...
		Convey("Index in a bolt store", func() {
			var err error
			sname := "boltStore"
			path := fmt.Sprintf("%s/test-%dk-%s.bleve", workingDir, loopNb, sname)
			server, err := NewDefaultServer(path)
		...
		Convey("Index in a scorch store", func() {
			var err error
			sname := "scorchStore"
			path := fmt.Sprintf("%s/test-%dk-%s.bleve", workingDir, loopNb, sname)
			server, err := NewDefaultServer(path)
